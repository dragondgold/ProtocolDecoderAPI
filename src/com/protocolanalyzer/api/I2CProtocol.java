...
public class I2CProtocol extends Protocol{
	...
	public void decode(double startTime) {
		...
		if(DEBUG) System.out.println("I2CDecode - I2C Protocol decode");
		if(clockSource == null) 
		...
		if(DEBUG) System.out.println("I2CDecode - Data lenght: "    +   logicData.length());
		if(DEBUG) System.out.println("I2CDecode - Clock lenght: "   +   clock.length());
		if(DEBUG) System.out.println("I2CDecode - BitSet data: "    +   logicData.toString());
		if(DEBUG) System.out.println("I2CDecode - BitSet clock: "   +   clock.toString());
		...
		while(index < logicData.length()) {
			...
			case startCondition:					
				...
				if(start != null){
					if(DEBUG) System.out.println("I2CDecode - Start Condition - index: " + index);
			...
			case readAddress:
				...
				if(data == null) return;
				try {
					if(DEBUG) System.out.println("I2CDecode - I2CAddress: " + Integer.toBinaryString(data[2] & 0xFF) + " -> " + data[2]);
					// Direccion
					...
					if(DEBUG) System.out.println("I2CDecode - IndexOutOfBoundsException - Returning");
					return;
				}
				break;
			...
			// Leo un byte de dato
			case readByte:		
				if(DEBUG) System.out.println("I2CDecode - Read Byte - index: " + index);
				I2Cstate = startCondition;	// Si por algun error tengo que salir va a ir a buscar un Start de nuevo
				
				data = readBits(index, 8);
				if(data == null) return;
				try{
					if(DEBUG) System.out.println("I2CDecode - I2CData: " + Integer.toBinaryString(data[2] & 0xFF) + " -> " + data[2]);
